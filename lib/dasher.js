...
function DasherButton(button) {
  ...
  this.dashButton.on("detected", function(mac_address) {
    if(!buttonList.hasOwnProperty(mac_address)){
        buttonList[mac_address] = 1;
    } else {
        buttonList[mac_address]++;
    }
    if(button.hasOwnProperty("addresskey")){
        if(options.headers==undefined){
            options.headers={};
        }
        options.headers[button.addresskey]=mac_address;
    }
    doLog(button.name + " pressed. Count: " +  buttonList[mac_address]+ " MAC Adress: "+mac_address);    
    if (debug){
